...
global.reloadHandler = async function (restatConn) {
  ...
  conn.sDesc = 'Description telah diubah menjadi \n@desc'
...
async function _quickTest() {
    ...
    if (!s.ffmpeg) {
        conn.logger.warn(`Sila install ffmpeg terlebih dahulu agar dapat menghantar video`)
    }

    if (s.ffmpeg && !s.ffmpegWebp) {
        conn.logger.warn('Sticker Mungkin Tak Beranimasi tanpa libwebp di ffmpeg (--enable-ibwebp while compiling ffmpeg)')
    }

    if (!s.convert && !s.magick && !s.gm) {
        conn.logger.warn('Fitur Stiker Mungkin Tak Bekerja Tanpa imagemagick dan libwebp di ffmpeg kerana belum terinstall (pkg install imagemagick)')
    }
...
setInterval(async () => {
  ...
  return [d, ' Hari ', h, ' Jam ', m, ' Minit ', s, ' Saat '].map(v => v.toString().padStart(2, 0)).join('')}
